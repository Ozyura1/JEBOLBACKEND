...
/// Admin IKD Detail Screen - View and manage single IKD submission.
class AdminIkdDetailScreen extends StatefulWidget {
  final String id;

  const AdminIkdDetailScreen({super.key, required this.id});

  @override
  State<AdminIkdDetailScreen> createState() => _AdminIkdDetailScreenState();
}

class _AdminIkdDetailScreenState extends State<AdminIkdDetailScreen> {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider.value(
      value: _provider,
      child: Scaffold(
        appBar: AppBar(title: const Text('Detail Pengajuan IKD')),
  ...
  Widget _buildBody(BuildContext context, AdminIkdProvider provider) {
    ...
    if (submission == null) {
      return const EmptyState(
        ...
        description: 'Pengajuan IKD tidak ditemukan atau telah dihapus.',
      );
    }
  ...
  Widget _buildStatusCard(BuildContext context, IkdSubmission submission) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          ...
          children: [
            ...
            _buildDetailRow('Jadwal Aktivasi', submission.formattedSchedule),
          ],
        ),
      ),
    );
  ...
  Widget _buildDataCard(BuildContext context, IkdSubmission submission) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
  ...
  Widget _buildLocationCard(BuildContext context, IkdSubmission submission) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
  ...
  Widget _buildAuditTrailCard(BuildContext context, IkdSubmission submission) {
  ...
  Widget _buildActionButtons(
    ...
    AdminIkdProvider provider,
    IkdSubmission submission,
  ) {
    final statusUpper = submission.status.toUpperCase();
    final canVerify = statusUpper == 'PENDING';
    final canSchedule = ['VERIFIED', 'PENDING'].contains(statusUpper);
    final canActivate = statusUpper == 'SCHEDULED';
      ...
      children: [
        ...
        if (canSchedule)
          ElevatedButton.icon(
            ...
            label: const Text('Jadwalkan Aktivasi'),
            style: ElevatedButton.styleFrom(
        ...
        if (canActivate)
          ElevatedButton.icon(
            onPressed: provider.isProcessing
                ? null
                : () => _showActivateDialog(context, provider, submission),
            icon: const Icon(Icons.smartphone_outlined),
            label: const Text('Tandai Teraktivasi'),
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.green,
              padding: const EdgeInsets.symmetric(vertical: 14),
            ),
          ),
        if (canActivate) const SizedBox(height: 12),
        if (canReject)
          OutlinedButton.icon(
            onPressed: provider.isProcessing
                ? null
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Verifikasi pengajuan IKD dari ${submission.nama}?'),
            const SizedBox(height: 16),
            TextField(
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          title: const Text('Jadwalkan Aktivasi'),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Jadwalkan aktivasi IKD untuk ${submission.nama}'),
              const SizedBox(height: 16),
              ListTile(
  ...
  void _showActivateDialog(
    ...
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Aktivasi IKD'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Tandai IKD ${submission.nama} sebagai teraktivasi?'),
            const SizedBox(height: 16),
            TextField(
              controller: catatanController,
              decoration: const InputDecoration(
                labelText: 'Catatan (opsional)',
                hintText: 'Tambahkan catatan aktivasi...',
              ),
              maxLines: 3,
            ),
          ],
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(context);
              await provider.activateSubmission(
                submission.id,
                catatan: catatanController.text.trim().isNotEmpty
                    ? catatanController.text.trim()
                    : null,
      ...
      builder: (context) => AlertDialog(
        ...
        content: Form(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Tolak pengajuan IKD dari ${submission.nama}?'),
              const SizedBox(height: 16),
              TextFormField(
